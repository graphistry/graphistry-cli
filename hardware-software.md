...
* CUDA driver rated for [NVIDIA RAPIDS](https://rapids.ai/) (CUDA 11.5+)
...
RAPIDS requires specific Nvidia drivers. Graphistry releases align with [Nvidia RAPIDS.ai](https://rapids.ai/) releases, so pick drivers compatible with the RAPIDS.ai distribution from the same time period. At time of writing, this would be CUDA 11.5 (RAPIDS 2023.x with PyTorch AI extensions)
