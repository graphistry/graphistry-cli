...
if [ -n "${SHIPYARD}" ] ; then
    ...
    for i in {1..12}
    do
        OUT=$(docker exec monolith-network-viz sh -c "${WGET_KEY} | sed 's#.*success\":\([a-z]*\).*#\1#p'")
        if [ "$OUT" = "true" ] ; then
            GRAPHISTRY_KEY=$(docker exec monolith-network-viz sh -c "${WGET_KEY} | sed 's#.*encrypted\":\([a-zA-Z0-9]*\).*#\1#p'")
