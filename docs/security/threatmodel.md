...
Graphistry is largely a standard enterprise webapp and uses modern design patterns, infrastructure, tools, & frameworks. It can be run through Graphistry Hub (Graphistry-hosted multi-tenant public cloud SaaS), automatically self-hosted in public clouds (AWS/Azure Marketplace single-VM AMIs), manually self-hosted in public cloud (`./graphistry` wrapper for docker compose, or Kubernetes), or self-hosted on-prem (including air-gapped). Details vary depending on distribution choice and configuration.
